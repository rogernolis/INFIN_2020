				...
				switch (input2){
					case '0':
						...
						do{																	//es demana introduir els segons de mostreig, si aquests són erronis, s'indicara i es demnara que es tornin a introduir
							...
							scanf("%s",&cadena[3]);
							i++;
						}while((cadena[3]=='2' && cadena[4]>'0')|| (cadena[3]>'2' && cadena[4]!= '\0')); 
						printf("Número de mostres fer la mitjana(1..9)?\n");								//es demana introduir el nombre de mostres per fer mitjana, nomes s'agafa una xifra(1-9). Es guarda en una altre cadena (cadena2)
						scanf("%s",&cadena2[0]);
						cadena2[1]='}';
